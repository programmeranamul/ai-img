...
function SingUp() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setLoading(false)
            navigate('/admin/singin');
            return toast.success("Account created. Login please!");
            ...
            setLoading(false)
            setAuth(false)
            return toast.error(e.response.data.error || "Try again");
        }
                        ...
                        <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                            ...
                            <form className="space-y-4 md:space-y-6" >
                                <div>
                                    ...
                                    <input type="text" name="name" className="bg-[#111] w-full text-white p-2.5 rounded-lg outline-none" placeholder="Your name" required={true} value={name} onChange={e => setName(e.target.value)} />
                                </div>
                                <div>
                                ...
                                <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                                    Don’t have an account yet? <Link to="/admin/singup" className="font-medium hover:underline text-white">Sign up</Link>
                                </p>
                            </form>
                        </div>
                    </div>
